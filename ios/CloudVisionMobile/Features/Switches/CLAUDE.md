...
| `InterfaceCountersService.swift` | Time-bounded fetch of per-port bitrates from the Turbine `analytics` dataset via `RouterV1.Get` on `{type: "device", name: "analytics"}` + `NetDBPaths.analyticsRates`. Values are **pre-derived float bps** (`inOctets` / `outOctets`); no client-side rate math. Sample thinning to ~60 points. Verified via Python gRPC probe + CV Web UI's `cv.worker.js` 2026-05-14. | Changing time window, adjusting thinning, swapping in the aggregated (1m / 15m) variant for longer windows, adding packet/error metrics |
