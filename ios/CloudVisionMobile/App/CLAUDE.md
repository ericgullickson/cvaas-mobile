...
| `RootView.swift` | Custom 4-tab shell (Devices / Alerts / Tools / More) and the `CustomTabBar` component. iOS 26's floating glass tab bar cannot be solid-tinted via standard appearance APIs, so we render our own navy bar. | Changing tab order, adding/removing tabs, modifying top-level navigation, debugging tab bar appearance |
